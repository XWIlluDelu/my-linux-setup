...
detect_installed_desktop_essentials() {
  dpkg_package_installed mpv \
    || dpkg_package_installed gnome-tweaks \
    || dpkg_package_installed gnome-shell-extension-manager
}
...
detect_installed_apps() {
  ...
  if supports_debian_apt_workflow "$update_pm"; then
    if detect_installed_desktop_essentials; then
...
collect_update_selection() {
  ...
  if ! has_interactive_tty; then
    info "No interactive terminal detected. Updating all detected apps and shell components."
    return 0
  fi
...
selected_updates_need_sudo() {
  if [[ "$INSTALL_DESKTOP_ESSENTIALS" -eq 1 || "$INSTALL_VSCODE" -eq 1 || "$INSTALL_EDGE" -eq 1 || "$INSTALL_FLATPAK" -eq 1 || "$INSTALL_WECHAT" -eq 1 || "$INSTALL_CLASH_VERGE_REV" -eq 1 || "$INSTALL_GHOSTTY" -eq 1 ]]; then
    return 0
  fi
...
selected_updates_need_apt_workflow() {
  ...
  [[ "$INSTALL_DESKTOP_ESSENTIALS" -eq 1 || "$INSTALL_VSCODE" -eq 1 || "$INSTALL_EDGE" -eq 1 || "$INSTALL_WECHAT" -eq 1 || "$INSTALL_CLASH_VERGE_REV" -eq 1 || "$INSTALL_ZOTERO" -eq 1 || "$INSTALL_OBSIDIAN" -eq 1 || "$INSTALL_GHOSTTY" -eq 1 ]]
}

selected_updates_need_package_manager() {
  if [[ "$INSTALL_SHELL_ENV" -eq 1 || "$INSTALL_DESKTOP_ESSENTIALS" -eq 1 || "$INSTALL_VSCODE" -eq 1 || "$INSTALL_EDGE" -eq 1 || "$INSTALL_FLATPAK" -eq 1 || "$INSTALL_CLASH_VERGE_REV" -eq 1 || "$INSTALL_GHOSTTY" -eq 1 ]]; then
    return 0
  fi
...
run_app_update_preflight() {
  ...
  if selected_updates_need_apt_workflow; then
    ...
    preflight_ok "No package-manager-bound app updates were selected."
  fi
  if selected_updates_need_sudo; then
    preflight_check_sudo_access
  else
    preflight_ok "No sudo-managed app updates were selected."
  fi
  if selected_updates_need_apt_workflow; then
    preflight_check_apt_locks
  else
    preflight_ok "No apt-managed app updates were selected, so apt lock checks were skipped."
  fi
  preflight_check_root_free_space 1048576 2097152 "App updates"
...
write_summary_file() {
  ...
  failed_count="$(app_update_failed_count)"
  ...
  {
    printf 'Linux Setup App Update Summary\n'
    printf 'Generated: %s\n' "$(date -Iseconds)"
...
if [[ "$RUN_MODE" != "apply" ]]; then
  ...
  printf '  2. %s/tasks/apps/install-apt-apps.sh --apply --desktop-essentials %s --vscode %s --edge %s\n' \
    ...
    "$INSTALL_VSCODE" \
    "$INSTALL_EDGE"
  printf '  3. %s/tasks/apps/install-external-apps.sh --apply --flatpak %s --wechat %s --clash-verge-rev %s --zotero %s --obsidian %s --ghostty %s --maple-font %s --miniforge %s\n' \
    "$ROOT_DIR" \
    "$INSTALL_FLATPAK" \
    "$INSTALL_WECHAT" \
...
if ! has_selected_apps; then
  info "No apps or shell components were selected for update."
  exit 0
fi
...
RUN_LOG="$CACHE_DIR/update-apps.log"
...
info "Updating selected apps and managed shell components."
  print_current_selection
...
if ! run_app_update_preflight; then
  write_summary_file
  die "App update stopped because preflight failed. Summary: $SUMMARY_FILE"
fi
...
if has_selected_packaged_apps; then
...
if has_selected_managed_apps; then
  if ! run_as_target_user "$TARGET_USER" "$TARGET_HOME" \
    bash "$ROOT_DIR/tasks/apps/install-external-apps.sh" \
    --apply \
...
if [[ "$failed_count" -gt 0 ]]; then
  printf '\n'
  warn "App update completed with ${failed_count} failed step(s)."
...
_del_logs="y"
if has_interactive_tty; then
  read -t 10 -p "Delete update logs in $CACHE_DIR? [Y/n] (auto-yes in 10s): " _del_logs || echo
fi

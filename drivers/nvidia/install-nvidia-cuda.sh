...
prompt_bool_whiptail() {
  local __var_name title prompt default_value status
  __var_name="$1"
  title="$2"
  prompt="$3"
  default_value="$4"
  ...
  if [[ "$default_value" -eq 1 ]]; then
    if whiptail --title "$title" --yesno "$prompt" 14 88; then
      status=0
    else
      status=$?
    fi
  else
    if whiptail --title "$title" --defaultno --yesno "$prompt" 14 88; then
      status=0
    else
      status=$?
    fi
...
prompt_bool() {
  ...
  if supports_whiptail_ui; then
    prompt_bool_whiptail "$__var_name" "$title" "$prompt" "$default_value"
  else
    prompt_bool_text "$__var_name" "$prompt" "$default_value"
  fi
}
...
run_deb_preflight() {
  ...
  prompt_bool continue_anyway \
    "Runfile Remnants" \
    "Possible runfile-managed NVIDIA driver remnants were found.\n\nContinue with the apt-managed driver path anyway?" \
    0
...
run_runfile_preflight() {
  ...
  if [[ "${#APT_NVIDIA_PACKAGES[@]}" -gt 0 ]]; then
    ...
    local purge_stack=0
    prompt_bool purge_stack \
      "Runfile Prep" \
      "Package-managed NVIDIA/CUDA packages were detected.\n\nPurge them before launching the runfile installer?" \
      0
  ...
  if [[ "$DISPLAY_MANAGER_ACTIVE" -eq 1 ]]; then
    ...
    prompt_bool stop_dm \
      ...
      "Display manager ${DISPLAY_MANAGER_UNIT} is active.\n\nStop it now before launching the runfile installer?" \
      1
...
collect_install_method() {
  ...
  choose_one INSTALL_METHOD \
    ...
    "$default_method" \
    "deb|package-managed|APT open driver + optional toolkit" \
    "run|NVIDIA runfile|Launch NVIDIA's own driver + CUDA installer" \
    "manual|preview only|Resolve packages and links without changing the system" \
    "skip|skip for now|Exit successfully without changing the system"
}
...
collect_driver_lock_preference() {
  ...
  prompt_bool LOCK_DRIVER_BRANCH \
    "Lock Driver Branch" \
    "Pin NVIDIA driver branch ${DRIVER_BRANCH}-open after installation?" \
    "$default_lock"
}
...
collect_toolkit_flag_for_deb() {
  ...
  prompt_bool INSTALL_TOOLKIT \
    "CUDA Toolkit" \
    "Install the CUDA toolkit now?" \
    "$default_flag"
}

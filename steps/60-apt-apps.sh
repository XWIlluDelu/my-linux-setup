...
# Community-standard scope: distro-managed APT/.deb apps are installed system-wide.
...
usage() {
  cat <<'EOF'
Install selected APT-managed desktop apps on Debian/Ubuntu.

Usage:
  60-apt-apps.sh [--check] [--apply]
...
if [[ "$DESKTOP_ESSENTIALS" -eq 0 && "$INSTALL_VSCODE" -eq 0 && "$INSTALL_EDGE" -eq 0 ]]; then
  info "No APT-managed desktop apps were selected."
  exit 0
fi
